...
Promotes `v0.2.0-candidate` to accepted after implementation, opaque-payloadType validation patch, validation review and merge into `develop`.

This version:

```text
1. Records MU-028 as Validated L4.
2. Records final validation: sovereign-connect 411/0/0/0 and EIB reports 56/0/0/0.
3. Records branch `feat/sc-b-mir-028-serialization-utilities-seed`, feature final commit `770ef9f` and merge commit `c8d50d0`.
4. Records the MIR-A outcome: broker-free SC-B serialization utilities exist before live NATS binding.
5. Records `scid1_` canonical ID token encoding / decoding and UUID correlation-token hex-no-hyphens encoding.
6. Records SC-JSON-WIRE-v1 envelope runtime, payloadType syntax validation, opaque preservation of unknown-valid payloadTypes and wire envelope validation.
7. Records the patch that explicitly verifies `sc.command.future.v1` is accepted by `WireEnvelopeValidator` despite not being a seed registry constant.
8. Preserves that MU-028 does not add NATS, JetStream, Testcontainers, NatsScBusPort, lifecycle runtime, productive ScdCommand or SC-D fact-family production classes.
```
...
Evidence package: docs/mir/mir-028/
Implementation branch: feat/sc-b-mir-028-serialization-utilities-seed
Feature final commit: 770ef9f
Merge commit: c8d50d0
Validation evidence:
  sovereign-connect module: 411 tests, 0 failures, 0 errors, 0 skipped
  EIB module reports present: 56 tests, 0 failures, 0 errors, 0 skipped
Review evidence:
  VALIDATION-SOV-SC-B-SERIALIZATION-UTILITIES-SEED-001-v0.2.0-patch-review.md
```

Validated MIR-A outcome:

```text
- `ScSubjectIdTokenCodec` implements `scid1_` canonical ID subject-token encoding / decoding;
- `ScCorrelationTokenCodec` implements 32-lowercase-hex UUID correlation token encoding;
- `ScSubjectTokenValidator` rejects invalid subject tokens and concrete-token wildcards;
- `ScWireEnvelopeKind` defines COMMAND / EVENT / RESPONSE;
- `ScJsonWireEnvelope` represents SC-JSON-WIRE-v1;
- `ScPayloadTypeRegistry` validates payloadType syntax and is not a closed semantic allowlist;
- `ScJsonWireCodec` roundtrips command, event and response wire envelopes;
- `WireEnvelopeValidator` validates wire invariants while preserving unknown-but-valid payloadTypes;
- no broker dependency, NATS implementation, lifecycle runtime or productive SC-D payload class is introduced.
```

Closed debt:

```text
DEBT-B-SER-001 — scid1_ encoder absent.
  disposition: closed by MU-028.

DEBT-B-SER-002 — correlation subject token codec absent.
  disposition: closed by MU-028.

DEBT-B-SER-003 — SC-JSON-WIRE-v1 envelope runtime absent.
  disposition: closed by MU-028.

DEBT-B-SER-004 — payloadType registry / syntax validator absent.
  disposition: closed by MU-028.

DEBT-B-SER-005 — wire envelope validator absent.
  disposition: closed by MU-028.
```
...
```text
This MIR does not implement NATS, JetStream, Testcontainers, NatsScBusPort, NATS subject builder,
lifecycle runtime, productive ScdCommand, SC-D fact-family production classes, provider execution,
semantic retry or terminal request-state ownership.
```
...
This MIR records the accepted and validated implementation attempt for the **SC-B Serialization Utilities Seed**.
